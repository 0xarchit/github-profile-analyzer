...
export function BaselineStats({ data }: { data: EngineResult }) {
  ...
  const stat = [
    ...
    { label: "Public Repos", value: fmt(num("1.2")) },
    { label: "Original Repos", value: origRatio != null ? `${Math.round(Number(origRatio) * 100)}%` : "--" },
    { label: "Total Stars", value: fmt(num("1.5")) },
    { label: "Total Forks", value: fmt(num("1.6")) },
    { label: "Followers/Following", value: ffRatio != null ? String(ffRatio) : "--" },
    { label: "Languages", value: langDistinct != null ? String(langDistinct) : "--" },
    { label: "Total Contributions", value: fmt(num("1.10")) },
    { label: "Total PRs", value: prsOpened != null ? String(prsOpened) : status("1.11") },
    { label: "Total Issues", value: issOpened != null ? String(issOpened) : status("1.12") },
    { label: "Longest Streak", value: longest != null ? `${longest}d` : "--" },
    { label: "Current Streak", value: currentStreak != null ? `${currentStreak}d` : "--" },
  ];
  const stat2 = [
    { label: "Active Days Ratio", value: adRatio != null ? `${Math.round(Number(adRatio) * 100)}%` : "--" },
    { label: "Profile README", value: readmePresent === true ? "Yes" : readmePresent === false ? "No" : "--" },
    { label: "Pinned Repos", value: fmt(pinnedCount) },
    { label: "Gists", value: fmt(num("1.21")) },
    { label: "Starred Repos", value: fmt(num("1.22")) },
    { label: "Profile Complete", value: completeCount != null && completeTotal != null ? `${completeCount}/${completeTotal}` : "--" },
    { label: "Total Releases", value: fmt(totalReleases) },
    { label: "Archived Repos", value: fmt(archivedCount) },
    { label: "Template Repos", value: fmt(templCount) },
    { label: "Years Active", value: fmt(yearsActive) },
  ];
